...
//TO_CUSTOMIZE: for part specific for the problem
...
public:
  ...
  public:
    ...
    {
    //TO_FILL:
    //P.S. component is referring to x(if 0), y(if 1), z(if 3)
      if (component == 0)
        return 0.0;
      else
        return 0.0;
  ...
  Stokes(const std::string  &mesh_file_name_,
         ...
         const std::function<double(const Point<dim> &)> sigma_,
           ...
           std::function<Tensor<1, dim>(const Point<dim> &)> f_
        #else
           std::function<Tensor<1, dim>(const Point<dim> &, const double &)> f_
        #endif
      )
...
protected:
  ...
  #ifndef TIME_DEPENDANT
  // Forcing term.
  std::function<Tensor<1, dim>(const Point<dim> &)> f;
  #else
