...
Stokes::assemble()
{
  ...
  std::vector<Vector<double>> solution_old_values(
    n_q, Vector<double>(dim+1));
  // Evaluation of the gradient of the old solution on quadrature nodes of
  // current cell.
  std::vector<std::vector<Tensor<1,dim>>> solution_old_grads(
    n_q,
      ...
      for (unsigned int q = 0; q < n_q; ++q)
        {
            ...
            #ifdef TIME_DEPENDANT
            Tensor<1, dim> f_loc  = f(fe_values.quadrature_point(q), time);
            #else
            Tensor<1, dim> f_loc  = f(fe_values.quadrature_point(q));
            #endif
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
            //TO_CUSTOMIZE:
            //P.S. phi_i = fe_values[velocity].value(i, q)
            // psi_i = fe_values[pressure].value(i, q)
            //do A = a(phi_i, phi_j), ...
            #ifdef TIME_DEPENDANT
            cell_matrix(i, j) += (1.0/delta_t)*
                ...
                fe_values.JxW(q);
                  ...
                  // Pressure mass matrix. Used for the preconditioner
                  cell_pressure_mass_matrix(i, j) +=
                    fe_values[pressure].value(i, q) *
                    fe_values[pressure].value(j, q) / mu_loc * fe_values.JxW(q);
                }
              cell_rhs(i) += f_loc*
                             fe_values[velocity].value(i, q) * //
                             fe_values.JxW(q);
            #ifdef TIME_DEPENDANT
              Tensor<1,dim> phi_i = fe_values[velocity].value(i,q);
              Tensor<1,dim> u_old;
              for (unsigned int d=0; d<dim; ++d)
                  u_old[d] = solution_old_values[q][d];

              cell_rhs(i) += (1.0/delta_t) * (phi_i * u_old) * fe_values.JxW(q);
            #endif
            }
        }
                  ...
                  for (unsigned int q = 0; q < n_q_face; ++q)
                    {
                      for (unsigned int i = 0; i < dofs_per_cell; ++i)
                        {
                        //TO_FILL
                        //constant/function(point) * (normal_vector * valocity_basis)
                        //P.S. the point coordinates are fe_face_values.quadrature_point(q), so fe_face_values.quadrature_point(q)[0] is the x coordinate
                            -0.0 *
